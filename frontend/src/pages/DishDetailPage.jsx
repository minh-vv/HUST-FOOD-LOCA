...
const COMMENTS_BASE_URL = "http://localhost:3000/comments";
...
export default function DishDetailPage() {
  ...
  const handleSubmitComment = async () => {
    ...
    if (!selectedRating) {
      setSubmitError("Hãy chọn số sao (1-5).");
      return;
    }
    if (!comment.trim()) {
      setSubmitError("Nội dung bình luận không được để trống.");
      return;
    }
  ...
  const submitEdit = async () => {
    if (!token) {
      setEditModal((prev) => ({ ...prev, error: "Vui lòng đăng nhập." }));
      return;
    }
    if (!editModal.rating) {
      setEditModal((prev) => ({ ...prev, error: "Hãy chọn số sao." }));
      return;
    }
  ...
  const submitDelete = async () => {
    if (!token) {
      setDeleteConfirm((prev) => ({ ...prev, error: "Vui lòng đăng nhập." }));
      return;
    }
    try {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {deleteConfirm.open && (
        <Modal onClose={() => setDeleteConfirm({ ...deleteConfirm, open: false })}>
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-gray-900">Xóa bình luận?</h3>
            <p className="text-sm text-gray-700">
              Bạn chắc chắn muốn xóa bình luận này? Thao tác không thể hoàn tác.
            </p>
            {deleteConfirm.error && (
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={deleteConfirm.loading}
              >
                {deleteConfirm.loading ? "Đang xóa..." : "Xóa"}
              </button>
            </div>
          </div>
        </Modal>
